...
class flexideque(BaseHolodeque[int, Any], ABC):
    ...
    def copy(self: Self) -> Self:
        ...
        new_holodeque._matrix = [[self._matrix[i][j] for j in range(self._shape)] for i in range(self._shape)]
        new_holodeque._shape = self._shape
        new_holodeque._element_list = [self._element_list[i] for i in range(len(self._element_list))]
        new_holodeque._element_map = {key: value for key, value in self._element_map.items()}
