...
def test_pushright_result_is_unique_to_parameter(lst):
    alphabet = set(lst)
    holodeques = [flexideque() for _ in alphabet]
    for i in lst:
...
def test_extendright(pair):
    ...
    for i in lst2:
        hd1.pushright(i)
    hd2 = flexideque(iterable=lst1 + lst2)
    assert hd1._matrix == hd2._matrix


@given(initial_list_strategy())
def test_copy(lst):
    hd1 = flexideque(iterable=lst)
    hd2 = hd1.copy()
    assert hd1 is not hd2
    for i in range(len(hd1._matrix)):
        for j in range(len(hd1._matrix[0])):
...
def test_extendleft(pair):
    ...
    for i in lst2:
        hd1.pushleft(i)
    hd2 = flexideque(iterable=list(reversed(lst2)) + lst1)
    assert list(hd1) == list(hd2)
